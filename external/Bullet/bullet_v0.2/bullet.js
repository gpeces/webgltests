/*
 * bullet.js (c) 2011 Peter Lueders <pl@jzone.de>
...
(function(Bullet) {
  ...
  Bullet.SimplexSolver.prototype.updateClosestVectorAndPoints=function() {
    ...
    switch (this.numVertices) {
      ...
      //ebug+='c'+ntos(this.cachedV.x);
      if (hasSeperation) {
        this.tmp1.scale2(this.cachedBC.barycentricCoords[0], this.simplexPointsP[0]);
        this.tmp2.scale2(this.cachedBC.barycentricCoords[1], this.simplexPointsP[1]);
        this.tmp3.scale2(this.cachedBC.barycentricCoords[2], this.simplexPointsP[2]);
        this.tmp4.scale2(this.cachedBC.barycentricCoords[3], this.simplexPointsP[3]);
        Bullet.VectorUtil.add5(this.cachedP1, this.tmp1, this.tmp2, this.tmp3, this.tmp4);
      
        this.tmp1.scale2(this.cachedBC.barycentricCoords[0], this.simplexPointsQ[0]);
        this.tmp2.scale2(this.cachedBC.barycentricCoords[1], this.simplexPointsQ[1]);
        this.tmp3.scale2(this.cachedBC.barycentricCoords[2], this.simplexPointsQ[2]);
        this.tmp4.scale2(this.cachedBC.barycentricCoords[3], this.simplexPointsQ[3]);
        Bullet.VectorUtil.add5(this.cachedP2, this.tmp1, this.tmp2, this.tmp3, this.tmp4);
  ...
  Bullet.GjkPairDetector.prototype.getClosestPoints=function(input,output,swapResults) {
    ...
    this.tmpNormalInB.set3(0,0,0);//Vector3f tmpNormalInB = Stack._alloc(Vector3f.class);
     ...
     for (;;) //while (true)
        ...
        //ebug+=this.cachedSeparatingAxis.x+' ';
        delta = this.cachedSeparatingAxis.dot(this.w);
        //ebug+=ntos(this.cachedSeparatingAxis.x)+' ';
  ...
  Bullet.ManifoldResult.prototype.addContactPoint=function(normalOnBInWorld,pointInWorld,depth) {
    ...
    if (isSwapped) {
      ...
      this.rootTransA.invXform(pointInWorld,this.localB);
    } else {
  ...
  Bullet.BroadphaseInterface.prototype.quantize3=function(out,point,isMax) {
    this.clampedPoint.set3(0,0,0);//point);//Vector3f clampedPoint = Stack._alloc(point);
  ...
  Bullet.BroadphaseInterface.prototype.sortMinUp=function(axis,edge,dispatcher,updateOverlaps) {
    var edgeArray = this.pEdges[axis];
    var pEdge_idx = edge;
    ...
    while (edgeArray.getHandle(pNext_idx) != 0 && (edgeArray.getPos(pEdge_idx) >= edgeArray.getPos(pNext_idx))) {
      var pHandleNext = this.getHandle(edgeArray.getHandle(pNext_idx));
        
      if (edgeArray.isMax(pNext_idx) != 0) {
        // if next edge is maximum remove any overlap between the two handles
        if (updateOverlaps) {
          var handle0 = this.getHandle(edgeArray.getHandle(pEdge_idx));
          var handle1 = this.getHandle(edgeArray.getHandle(pNext_idx));
  ...
  Bullet.BroadphaseInterface.prototype.sortMaxUp=function(axis,edge,dispatcher,updateOverlaps) {
    var edgeArray = this.pEdges[axis];
    var pEdge_idx = edge;
    var pNext_idx = pEdge_idx + 1;
    var pHandleEdge = this.getHandle(edgeArray.getHandle(pEdge_idx));
  ...
  Bullet.BroadphaseInterface.prototype.updateHandle=function(handle,aabbMin,aabbMax,dispatcher) {
    ...
    for (var axis = 0; axis < 3; axis++) {
      ...
      //if (handle==2&&axis==1) ebug+=aabbMin+' - '+aabbMax+' . ';
      //if ((dmin!=0)||(dmax!=0)) {
        ////alert('dmin dmax !=0');
        //ebug+='dmin='+dmin+' dmax='+dmax;
      //}
      if (dmin < 0) {
  ...
  Bullet.ContactSolverInfo.prototype.erp = 0.2;//0.05; //pl:was 0.2, for less bouncing../pl used as Baumgarte factor
              ...
              var frictionIndex = this.tmpSolverConstraintPool.length;
              {
                ...
                if ((infoGlobal.solverMode & Bullet.SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
                  ...
                  if (rb0 != null) {
                    ...
                    //if (this.tmpSolverBodyPool[solverConstraint.solverBodyIdA]==undefined) {
                    //  alert("undef "+solverConstraint.solverBodyIdA);
                    //}
                  ...
                  if (rb1 != null) {
                    ...
                    //if (this.tmpSolverBodyPool[solverConstraint.solverBodyIdB]==undefined) {
                    //  alert("undef "+solverConstraint.solverBodyIdB);
                    //}
